...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      // Try updating crm_clients first (prefer code match over id to avoid mismatches)
      let [existing] = await pool.query('SELECT id FROM crm_clients WHERE code = ?', [clientId]);
      if ((existing as any[]).length === 0) {
        const parsedId = parseInt(clientId);
        const rawId = parsedId >= 10000 ? parsedId - 10000 : parsedId;
        [existing] = await pool.query('SELECT id FROM crm_clients WHERE id = ? OR id = ?', [parsedId, rawId]);
      }
      if ((existing as any[]).length > 0) {
        ...
        const map: Record<string, string> = {
          ...
          bankName: 'bank_name', bankAccount: 'bank_account', bankCurrency: 'bank_currency', bankAccountType: 'bank_account_type',
